...
def send_welcome(message: Message):
    ...
    bot.send_message(message.from_user.id, text=f'{form.hbold("Access Bars")} помог мне вывести мои знания на новый уровень. '
                                            ...
                                            f'У меня намерение {form.hitalic("развиваться")} в данном направлении и делиться своими знаниями с миром.',
                        parse_mode='HTML')
    time.sleep(3)
    bot.send_message(message.from_user.id, text=f'Я буду рада встретиться с каждым из Вас:\n'
                                            f'- {form.hbold("на сессии")} (возможно, это будет лучший энергетический массаж в Вашей жизни :)),\n'
